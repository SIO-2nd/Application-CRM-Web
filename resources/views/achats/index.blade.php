...
<a href="{{ route('rendezvous.index') }}">Rendez-Vous</a>
        ...
        <a href="{{ route('achats.index') }}" class="active">Achats</a>
...
<td>{{ $value->Qte }}</td>
<td>
<form action="{{ route('achats.destroy', $value->IdAchat) }}" method="POST">
<a class="btn btn-info button button2 button_action" href="{{ route('achats.show', $value->IdAchat) }}"><i class="fas fa-tags"></i></a>
<a class="btn btn-primary button button3 button_action" href="{{ route('achats.edit', $value->IdAchat) }}"><i class="fas fa-pen"></i></a>
@csrf
@method('DELETE')
...
@endforeach
<form action="/search-achat" method="get">
<input type="search" id="search" style="border-radius: 4px; margin-right: 6px; width: 30%; background-color: #D1D5E4; padding-top: 3px; padding-bottom: 3px; padding-left: 12px; border: none;" placeholder="Rechercher..." name="search" required>
    <span>
        ...
        @if (isset($search))
            <a href="/achats" class="btn btn-sm btn-default">{{ $search }}<span id="close" style="margin-left: 6px;"><i class="fas fa-times"></i></span></a>
        @endif
    </span>
</form>
<br><br><br>
...
{{ $data->links() }}
<a class="btn btn-primary button button3 button_bottom" href="{{ route('achats.create') }}"><i class="fas fa-plus"></i> Ajouter un nouvel achat</a>
</div>
